...
$sql = "SELECT s.name, count(u.id) as schoolcount from {newwaves_schools} s left join {newwaves_schools_users} u on
                             u.schoolid=s.id where role = 'headadmin' group by s.name";
...
<!--Load the AJAX API-->
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Schools with Number of Headadmin',
          width:'600',
          height:'400'
        };

        var chart = new google.visualization.PieChart(document.getElementById('school_headadmin'));
        chart.draw(data, options);
      }
    </script>

<div id="school_headadmin"></div>
