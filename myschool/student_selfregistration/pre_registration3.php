...
$PAGE->set_url(new moodle_url('/local/newwaves/myschool/student_selfregistration/pre_registration.php'));
...
if ($mform->is_cancelled()){
    redirect($CFG->wwwroot.'/local/newwaves/myschool/student_selfregistration/pre_registration1.php', 'No School Student is created. You cancelled the operation.');
    ...
    $schoolinfo_href = "preregistration3.php?q=".mask($_SESSION['school_id']);
    $newStudent = $fromform->surname.' '.$fromform->firstname;
    redirect($CFG->wwwroot."/local/newwaves/myschool/studentselfregistration/{$schoolinfo_href}", "A Student with the name <strong>{$newStudent}</strong> has been successfully created.");
    ...
    if (!isset($_GET['q']) || $_GET['q'] == '') {
        redirect($CFG->wwwroot. '/local/newwaves/myschool/studentselfregistration/preregistration1.php', 'Sorry, the page is not fully formed with the required information.');
    }
...
<div class="row mt-4">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <?php
                $submit_href = "window.location='complete_registration.php?q=".mask($_SESSION['schoolId'])."'";
            ?>
