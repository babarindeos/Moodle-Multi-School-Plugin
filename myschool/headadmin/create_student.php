...
$PAGE->navbar->add(get_string('myschoolheadadmincreatestudent', 'local_newwaves'), new moodle_url('/local/newwaves/myschool/headadmin/create_student.php?q='.$_SESSION['schoolid']));
...
if ($mform->is_cancelled()){
    redirect($CFG->wwwroot.'/local/newwaves/myschool/headadmin/create_student.php', 'No Student is created. You cancelled the operation.');
...
//            $gender = gender($fromform->gender);
            if ($fromform->gender==0){
                ...
                $create_student_href = "create_student.php?q=".mask($fromform->school_id);
                redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$create_student_href}", 'Gender has not been selected. Please select Gender option.', null, \core\output\notification::NOTIFY_ERROR );
            }
            ...
            if ($fromform->class==0){
                ...
                //$_GET_URL_school_id = $fromform->school_id;
                $create_student_href = "create_student.php?q=".mask($fromform->school_id);
                redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$create_student_href}", 'Class has not been selected. Please select Class option.', null, \core\output\notification::NOTIFY_ERROR );
            ...
            if ($fromform->gender!=0 && $fromform->class!=0){
                      ...
                      $manage_students_href = "manage_students.php?q=".mask($fromform->school_id);
                      $newStudent = $fromform->surname.' '.$fromform->firstname;
                      redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$manage_students_href}", "A Student with the name <strong>{$newStudent}</strong> has been successfully created.");
