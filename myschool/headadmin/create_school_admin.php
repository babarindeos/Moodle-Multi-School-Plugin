...
if ($mform->is_cancelled()){
    ...
    if ($isEmailExist>0){
            ...
            redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$create_school_admin_href}", "<strong>[Duplicate Email Error]</strong> A user record with that email <strong>{$email}</strong> already exist.");
    }else{
...
//            $gender = gender($fromform->gender);
            if ($fromform->title==0){
                ...
                $create_school_admin_href = "create_school_admin.php?q=".mask($fromform->school_id);
                redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$create_school_admin_href}", 'Title has not been selected. Please select Title option.', null, \core\output\notification::NOTIFY_ERROR );
            }
            ...
            if ($fromform->gender==0){
                ...
                //$_GET_URL_school_id = $fromform->school_id;
                $create_school_admin_href = "create_school_admin.php?q=".mask($fromform->school_id);
                redirect($CFG->wwwroot."/local/newwaves/myschool/headadmin/{$create_school_admin_href}", 'Gender has not been selected. Please select Gender option.', null, \core\output\notification::NOTIFY_ERROR );
...
echo $OUTPUT->header();
echo "<h2>Create School Admin</small></h2>";
