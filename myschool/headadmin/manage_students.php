...
global $DB;
 ...
 $PAGE->navbar->add(get_string('myschoolheadadminmanagestudents', 'local_newwaves'), new moodle_url('/local/newwaves/myschool/headadmin/manage_students.php?q='.$_GET_URL_school_id));
 ...
 <div class="row d-flex justify-content-right mt-2 mb-4">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <?php
           $create_student_href = "create_student.php?q=".mask($_GET_URL_school_id);
        ?>
        <button onClick="window.location='<?php echo $create_student_href; ?>'" class='btn btn-sm btn-primary rounded'><i class='fas fa-user-graduate'></i> Create Student</button>
    </div>
 </div>
 ...
 <?php
  ...
  echo "<tr class='font-weight-bold' >";
       echo "<th class='py-3'>SN</th><th>Admission No.</th><th>Name</th><th>Email</th><th>Phone</th><th class='text-center'>Action</th></tr>";
  echo "</thead>";
  echo "<tbody>";
        foreach($headadmin as $row){
            ...
            $btnView =  "<button onclick={$viewHref} class='btn btn-success btn-sm rounded '><i class='fas fa-search'></i> <small>View</small></button>";
            $btnEdit =  "<button title='Edit Student' onclick={$editHref} class='btn btn-warning btn-sm rounded '><i class='far fa-edit'></i> <small>Edit</small></button>";
            $btnDelete = "<button title='Delete Student' id='btn{$row->id}' class='btn btn-danger btn-sm rounded btn-delete' data-toggle='modal' data-target='#deleteModalCenter'>
