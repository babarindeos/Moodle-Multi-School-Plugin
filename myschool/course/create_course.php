...
if ($mform->is_cancelled()){
    redirect($CFG->wwwroot.'/local/newwaves/schools/create_course.php', 'No School Student is created. You cancelled the operation.');
...
$_GET_URL_school_id = $_GET_URL_school_id[1];
          ...
          $DB->insert_record('newwaves_course', $recordtoinsert);
          ...
          $DB->insert_record("course", $createlogin);
//
//
//          //
//          // write to teacher
...
//          $DB->insert_record("newwaves_schools_teachers", $createteacher);
    ...
    $schoolinfo_href = "manage_course.php?q=".mask($_GET_URL_school_id);
    $newStudent = $fromform->name;
    redirect($CFG->wwwroot."/local/newwaves/myschool/course/{$schoolinfo_href}", "A Course with the name <strong>{$newStudent}</strong> has been successfully created.");
//    }
    ...
    if (!isset($_GET['q']) || $_GET['q'] == '') {
        redirect($CFG->wwwroot . '/local/newwaves/headadmin/headadmin_dashboard.php', 'Sorry, the page is not fully formed with the required information.');
    }
    $_GET_URL_school_id = explode("-", htmlspecialchars(strip_tags($_GET['q'])));
    $_GET_URL_school_id = $_GET_URL_school_id[1];
...
<div class="row d-flex justify-content-right mt-4 mb-4">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
        <h4 class='font-weight-normal'>Create Course</h4>
    </div>
</div>
