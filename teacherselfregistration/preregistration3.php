...
if ($mform->is_cancelled()){
    redirect($CFG->wwwroot.'/local/newwaves/teacherselfregistration/preregistration1.php', 'No School Teacher is created. You cancelled the operation.');
...
//        $DB->insert_record("newwaves_schools_teachers", $createteacher);
    ...
    redirect($CFG->wwwroot."/local/newwaves/teacherselfregistration/{$schoolinfo_href}", "A Teacher with the name <strong>{$newTeacher}</strong> has been successfully created.");
//    }
    ...
    if (!isset($_GET['q']) || $_GET['q'] == '') {
        redirect($CFG->wwwroot . '/local/newwaves/teacherselfregistration/preregistration1.php', 'Sorry, the page is not fully formed with the required information.');
    }
